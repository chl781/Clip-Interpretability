...
def display_image(testset, img_idx, ax, title, wnid_to_class):
    ...
    ax.set_title(title, fontsize=20)  # Set the title for the subplot
...
def visualize_radial_streak(U_rotated, title = 'Pairs plot for U_rotated', nrows=5, ncols=5, save_path=None):
    fig, axs = plt.subplots(nrows, ncols, figsize=(40, 40))
    for i in range(nrows):
        for j in range(ncols):
            img_idx = i * ncols + j  # Calculate the index of the image to display
            ax = plt.subplot(nrows, ncols, img_idx + 1)  # Create a subplot
            if i != j:
                plt.scatter(U_rotated[:,i], U_rotated[:,j], s=20)
                ax.set_xlabel('PC {}'.format(i), fontsize=20)
                ax.set_ylabel('PC {}'.format(j), fontsize=20)
...
def update_clade_names(clade, interpret_cluster):
    if clade.name in interpret_cluster:
        clade.name = clade.name+': '+interpret_cluster[clade.name]
    for sub_clade in clade.clades:
        update_clade_names(sub_clade, interpret_cluster)

def parse_twig(twig):
    parts = twig.split(')')
    label = parts[1]
    children_str = parts[0][1:]  # Remove the opening '('
    children_parts = children_str.split(',')
    children = []
    for child in children_parts:
        node_name, distance = child.split(':')
        children.append((node_name, float(distance)))
    return label, children

def build_clade(twig, clades):
    label, children = parse_twig(twig)
    child_clades = []
    for child_name, distance in children:
        if child_name in clades:
            child_clade = clades[child_name]
        else:
            child_clade = Clade(name=child_name, branch_length=distance)
        child_clade.branch_length = distance
        child_clades.append(child_clade)
    parent_clade = Clade(clades=[], name=label)
    parent_clade.clades.extend(child_clades)
    clades[label] = parent_clade

def plot_phylogenetic_tree(twigs, figsize=(10, 10), update_name=False, interpret_cluster=None):
    clades = {}
    for twig in twigs:
        build_clade(twig, clades)
    tree = Phylo.BaseTree.Tree(root=clades[twigs[-1].split(')')[1]])
    tree.ladderize()
    fig = plt.figure(figsize=figsize, dpi=100)  # Create a figure with the desired size
    axes = fig.add_subplot(1, 1, 1)
    axes.set_xlabel('branch length', fontsize = 25)
    axes.set_ylabel('taxa', fontsize = 25)
    plt.rcParams.update({'font.size': 20})
    if update_name:
        update_clade_names(tree.root, interpret_cluster=interpret_cluster)
    Phylo.draw(tree, axes=axes, do_show=False, label_colors=color_func)
    plt.show()
